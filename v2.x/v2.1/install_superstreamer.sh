...
def stream():
    try:
        d=json.loads(sh(f"curl -s {MTX_API}"))
        for p in d.get("items",[]):
            if p["name"]=="usb" and p["ready"]:
...
def res():
    for l in open(CONF):
        if l.startswith("RES="):
            return jsonify(res=l.strip().split("=")[1])
...
<div class="bg-gray-800 p-4 rounded-xl text-center shadow-lg">
  <h1 class="text-xl font-semibold tracking-wide">
    PiStream-Lite <span class="text-blue-400">v2.1</span>
  </h1>
